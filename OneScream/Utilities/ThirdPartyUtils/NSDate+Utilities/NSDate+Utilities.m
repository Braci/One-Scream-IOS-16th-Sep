...
{
    NSComparisonResult compare = [[NSCalendar currentCalendar]compareDate:self toDate:aDate toUnitGranularity:NSCalendarUnitDay];
...
-(NSString *)toString
{
    ...
    if([self isToday])
        dateString = [NSString stringWithFormat:@"Today at %@",[dateFormmater stringFromDate:self]];
    else if ([self isTomorrow])
        dateString = [NSString stringWithFormat:@"Tomorrow at %@",[dateFormmater stringFromDate:self]];
    else if ([self isYesterday])
        dateString = [NSString stringWithFormat:@"Yesterday at %@",[dateFormmater stringFromDate:self]];
    else
    {
        if(self.hour==0)
            dateFormmater.dateFormat = @"dd MMM YYY";
        else
            dateFormmater.dateFormat = @"dd MMM YYYY at hh:mm";
        dateString = [dateFormmater stringFromDate:self];
    }
    return dateString;
}
@end